...
class Coches:
    ...
    #El encapsulamiento en Python tambien es llamado visibilidad y prácticamente se puede hacer que un atributo y/o método sean públicos o privados
...
class camiones(Coches):
    def _init_(self, color, marca, modelo, velocidad, caballaje, plazas, eje, capacidadCarga):
        ...
        self.__eje = eje  # Atributo privado __eje
        self.__capacidadCarga = capacidadCarga  # Atributo privado __capacidadCarga
      
    __tipo_carga = 0  # Atributo de clase privado __tipo_carga
    ...
    def getInf(self):
       print(f"Marca: {self.getMarca()} {self.getColor()}, numeros de plazas: {self.getPlazas()} \nModeloself.getModelo() con una velocidad de {self.getVelocidad()} Km/h y un potencia de {self.getCaballaje()} hp, con {self.getEje()} con una capacidad de carga de {self.getCapacidadCarga()} metros cubicos")
...
class camionetas(Coches):
    def _init_(self, color, marca, modelo, velocidad, caballaje, plazas, traccion, cerrada):
        ...
        self.transportar = ""  # Inicialización del atributo transportar
